 ...
 */
function cookingTime(neededPower, minutes, seconds, microwavePower) {
    let neededPowerValue = parseInt(neededPower.replace('W', ''), 10);
    let microwavePowerValue = parseInt(microwavePower.replace('W', ''), 10);

    let totalSeconds = minutes * 60 + seconds;

    let adjustedSeconds = Math.ceil(totalSeconds * neededPowerValue / microwavePowerValue);

    let adjustedMinutes = Math.floor(adjustedSeconds / 60);
    let adjustedRemainingSeconds = adjustedSeconds % 60;

    return `${adjustedMinutes} minutes ${adjustedRemainingSeconds} seconds`;
}
